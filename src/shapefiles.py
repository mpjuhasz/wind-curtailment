...
def shp_to_coords(df: gpd.GeoDataFrame, transform_coords: bool) -> list[gpd.GeoDataFrame]:
    dfs = []
    ...
    if not lines.empty:
        lines["latlongs"] = lines.apply(lambda x: {"coords": [bng_xy_to_lat_long(c[0], c[1]) if transform_coords else c for c in x["geometry"].coords]}, axis=1)
        dfs.append(lines)
    
    if not polygons.empty:
        polygons["latlongs"] = polygons.apply(lambda x: {"coords": [bng_xy_to_lat_long(c[0], c[1]) if transform_coords else c for c in x["geometry"].exterior.coords]}, axis=1)
        dfs.append(polygons)
    ...
    if not multi_polygons.empty:
        ...
        multi_polygons["latlongs"] = multi_polygons.apply(lambda x: {"coords": [bng_xy_to_lat_long(c[0], c[1]) if transform_coords else c for c in x["geometry"].exterior.coords]}, axis=1)
    ...
    if not multilines.empty:
        ...
        multilines["latlongs"] = multilines.apply(lambda x: {"coords": [bng_xy_to_lat_long(c[0], c[1]) if transform_coords else c for c in x["geometry"].coords]}, axis=1)
...
def main(input_path: str, output_path: str, transform_coords: bool):
    ...
    for file in [input_path]:
        ...
        dfs.extend(
            shp_to_coords(df, transform_coords)
        )
